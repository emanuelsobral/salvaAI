...
O backend agora preserva HTTP 503 quando o Gemini retorna indisponibilidade, em vez de convertê-lo em 502. A interface informa que é necessário tentar novamente mais tarde. Em HTTP 503, há uma única repetição após 1 segundo, com o mesmo modelo e conteúdo. As duas tentativas compartilham o limite total de 45 segundos. Não há repetição em 429, erro de configuração, timeout ou resposta já gerada, nem troca automática de modelo. Cada solicitação do usuário pode fazer até duas chamadas ao provedor. O diagnóstico local confirma indisponibilidade nas duas chamadas testadas; a configuração e a resposta de uma chamada autenticada na Netlify ainda podem diferir. Os demais diagnósticos distinguem acesso recusado, modelo não encontrado, bloqueio de conteúdo e limite de geração.
