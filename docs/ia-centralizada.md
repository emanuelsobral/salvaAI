...
O backend agora preserva HTTP 503 quando o Gemini retorna indisponibilidade, em vez de convertê-lo em 502. A interface informa que é necessário tentar novamente mais tarde. Não há repetição automática nem troca automática de modelo. Cada solicitação faz no máximo uma chamada ao provedor. Na mesma aba, chat, insights e recibos compartilham uma pausa local de 60 segundos após 429 e 30 segundos após 503; somente uma chamada pode ficar em andamento. Recarregar a página reinicia essa proteção local. Esses intervalos reduzem novas tentativas e não garantem a recuperação do serviço ou a renovação da cota. Os limites do servidor continuam ativos. O diagnóstico local confirma indisponibilidade nas duas chamadas testadas; a configuração e a resposta de uma chamada autenticada na Netlify ainda podem diferir. Os demais diagnósticos distinguem acesso recusado, modelo não encontrado, bloqueio de conteúdo e limite de geração.

A mensagem "A cota de IA foi atingida" é emitida ao receber 429 do Gemini. Consulte os limites ativos por projeto e modelo no Google AI Studio: podem envolver requisições por minuto, tokens por minuto e requisições diárias. A resposta mostrada não identifica qual limite foi atingido. Uma pausa de 60 segundos não resolve cota diária esgotada. Referência: https://ai.google.dev/gemini-api/docs/rate-limits
